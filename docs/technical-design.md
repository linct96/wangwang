...
- `sources`：外部订阅 URL、待验证 URL、刷新间隔、条件请求头、状态与错误；固定的 `system-manual` 记录仅用于手动节点关系。
...
来源地址修改先写入 `pending_url`，保存不会自动刷新订阅；用户可通过刷新操作触发验证并在成功后提升为正式 URL。失败时清除待验证值并保留旧 URL 和旧节点。来源启停、删除及手动节点增删改都会重新生成受影响配置。
