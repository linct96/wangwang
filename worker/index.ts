...
import { constantTimeEqual, assertRemoteUrl, subscriptionToken, validOrigin, verifyAccess } from './security'
...
app.use('*', async (c, next) => {
  ...
  if (hostname !== 'localhost' && hostname !== '127.0.0.1' && c.env.APP_HOST && hostname !== c.env.APP_HOST) return c.notFound()
  await next()
})

app.use('/admin/api/*', async (c, next) => {
  if (!(await verifyAccess(c.req.raw, c.env))) return fail(c, 401, 'ACCESS_REQUIRED', '需要通过 Cloudflare Access 登录')
