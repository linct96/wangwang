...
function normalizeRegex(value: string | null, subject: string, label: string) {
  ...
  if (normalized.length > 200) throw new Error(`${subject}的${label}正则不能超过 200 个字符`)
  try {
    new RegExp(normalized)
  } catch {
    throw new Error(`${subject}的${label}正则无效`)
  }
  return normalized
}

async function validateReferences(env: Env, sourceIds: string[], nodeIds: string[]) {
...
export async function validateProfileNodeBinding(env: Env, binding: ProfileNodeBindingInput) {
  if (binding.mode === 'source') {
    const sourceIds = [...new Set(binding.sourceIds)]
    if (!sourceIds.length) throw new Error('节点选择至少需要一个节点源')
    const normalized = {
...
export async function validateProfileSlotBindings(
  ...
  await validateReferences(env, sourceIds, nodeIds)
  return normalized
}
...
export async function sourceRequiredByProfile(env: Env, sourceId: string, enabledOnly: boolean) {
  const alternative = enabledOnly ? sql`AND s.enabled = 1` : sql``
  const row = await db(env).get(sql`
    SELECT 1 FROM (
      SELECT target.profile_id
      FROM profile_node_sources target
      WHERE target.source_id = ${sourceId}
        AND NOT EXISTS (
          SELECT 1
          FROM profile_node_sources other
